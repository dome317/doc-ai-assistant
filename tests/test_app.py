"""Unit tests for DocAI Assistant core functions."""
...
class TestSafeLatin1:
    ...
    def test_german_umlauts(self):
        result = app.safe_latin1("Ventilation Heat Recovery")
        assert "Ventilation" in result

    def test_emoji_replaced(self):
        result = app.safe_latin1("Test \U0001f300 Emoji")
        assert "\U0001f300" not in result
...
class TestProductToText:
    def test_basic_product(self):
        product = {
            "name": "ELS NFC",
            "category": "Single-Duct Ventilation System",
            "description": "Multifunctional ventilator insert",
            "use_cases": ["Bathroom", "WC"],
            "highlights": ["NFC configuration"],
            "specs": {
                "airflow_m3h": [7.5, 100],
        ...
        assert "7.5-100" in result
        assert "26-47 dB(A)" in result
        assert "NFC configuration" in result
...
# ---------------------------------------------------------------------------
# DocAIReport (PDF generation)
# ---------------------------------------------------------------------------


class TestDocAIReport:
