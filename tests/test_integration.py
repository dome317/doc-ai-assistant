"""
Integration tests for DocAI Assistant — validates expected outputs.
...
class TestDemoExtraction:
    ...
    def test_document_type_valid(self, demo_extraction):
        valid_types = {"datasheet", "building_plan", "customer_inquiry",
                       "installation_protocol", "tender", "other"}
...
# ---------------------------------------------------------------------------
# 3. Demo Search Results Validation
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# 5. NFC Configuration Validation
# ---------------------------------------------------------------------------
...
class TestNFCConfig:
    def test_default_config_structure(self):
        ...
        for key in ["stage_1_m3h", "stage_2_m3h", "stage_3_m3h"]:
            val = config["airflow_config"][key]
            assert val in app.ELS_NFC_PARAMS["airflow_steps_m3h"]
        assert config["airflow_config"]["stage_1_m3h"] < config["airflow_config"]["stage_2_m3h"]
        assert config["airflow_config"]["stage_2_m3h"] < config["airflow_config"]["stage_3_m3h"]
    ...
    def test_factory_defaults_ascending(self):
        ...
        assert defaults["stage_1"] < defaults["stage_2"] < defaults["stage_3"]
...
# ---------------------------------------------------------------------------
# 7. PDF Report Content Validation
# ---------------------------------------------------------------------------
...
class TestSemanticSearch:
    ...
    def test_voc_query_returns_voc_first(self, chroma):
        results = self._search(chroma, "VOC sensor office air quality")
        top_id = results["ids"][0][0]
        assert top_id == "ELS-NFC-VOC", f"Expected ELS-NFC-VOC, got {top_id}"

    def test_feuchte_query_returns_f_variant(self, chroma):
        results = self._search(chroma, "Humidity sensor bathroom shower wet room")
...
# ---------------------------------------------------------------------------
# 9. Live API Tests (only with key)
# ---------------------------------------------------------------------------
...
class TestLiveAPI:
    ...
    def test_extraction_returns_valid_json(self):
        ...
        test_input = (
            "Technical Datasheet ELS NFC VOC\n"
            "Article No.: 40764\n"
            "Airflow: 100 m3/h\n"
            "Sound pressure level: 47 dB(A)\n"
            "Protection class: IPX5\n"
            "Safety class: II\n"
            "NFC configuration: Yes\n"
        )
    ...
    def test_search_ranking_returns_valid_json(self):
        ...
        user_msg = (
            "Requirement: Quiet ventilator for 25m2 office with air quality sensor\n\n"
            "Candidates:\n"
            "- ELS NFC VOC: VOC sensor, 26-47 dB(A), NFC\n"
            "- ELS NFC CO2: CO2 sensor, 26-47 dB(A), NFC\n"
            "- ELS NFC: Base without sensor, 26-47 dB(A), NFC\n"
        )
